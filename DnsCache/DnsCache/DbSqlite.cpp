...
{
   m_stmt = NULL;
   if (SQLITE_OK != _sqlite3_prepare(db, statement, -1, &m_stmt, 0))
   {
	   m_stmt = NULL;
   }
}
...
bool CSqlStatement::Bind(int pos_zero_indexed, LPCTSTR value)
{
   ...
   return (SQLITE_OK == _sqlite3_bind_text(m_stmt, pos_zero_indexed+1, v, v.GetLength(), SQLITE_TRANSIENT));
}

bool CSqlStatement::Bind(int pos_zero_indexed, double value)
{
   return (SQLITE_OK == _sqlite3_bind_double( m_stmt, pos_zero_indexed+1,value));
}

bool CSqlStatement::Bind(int pos_zero_indexed, int value)
{
   return (SQLITE_OK == _sqlite3_bind_int( m_stmt, pos_zero_indexed+1, value));
}

bool CSqlStatement::BindNull(int pos_zero_indexed)
{
   return (SQLITE_OK == _sqlite3_bind_null( m_stmt, pos_zero_indexed+1));
}

bool CSqlStatement::Execute()
{
   if (SQLITE_DONE == _sqlite3_step(m_stmt))
   {
...
{
   return LPCTSTR(_sqlite3_column_name(m_stmt, pos_zero_indexed));
}
...
time_t CSqlStatement::ValueLong(int index)
{
	return _sqlite3_column_int64(m_stmt, index);
}
...
{
   return (SQLITE_ROW == _sqlite3_step(m_stmt));
}
...
{
   m_strDbFileName = db_file;

   sqlite3_config(SQLITE_CONFIG_SERIALIZED);//串行模式,sqlite内部启用了所有的锁
   return (SQLITE_OK == _sqlite3_open(db_file, &m_pDb));
} 
...
//#endif
//
//   rc = sqlite3_exec(m_pDb,lpsz,SelectCallback,static_cast<void*>(&res),&errmsg);
//
//   if ( rc != SQLITE_OK )
//   {
//      fResult = false;
...
{
   return new CSqlStatement(statement, m_pDb);
}

LPCTSTR CDbSQLite::LastError()
{
   return (LPCTSTR)_sqlite3_errmsg(m_pDb);
}
